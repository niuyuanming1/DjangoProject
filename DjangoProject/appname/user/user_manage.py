...
def add_user(request):
    ...
    num_email = Users.objects.filter(email=email)
    num_userName = Users.objects.filter(userName=userName)
    if len(num_email) < 1 and len(num_userName) < 1:
        ...
        user = Users(email=email, passWord=passWord, userName=userName, phoneNumber=phoneNumber,
...
def login_user(request):
    ...
    sql_email = Users.objects.filter(email=userName)
    if len(sql_email) >= 1:
...
def select_active_user(request):
    ...
    sql_active_user = Users.objects.filter(email=active_user_email)
...
def delete_user(request):
    user1 = Users.objects.get(id=eval(request.body.decode())["id"])
    user1.delete()
    return True
